...
-- ตัวควบคุมฟิสิกส์ยุคใหม่ (Constraint Physics)
local att, lv, ao = nil, nil, nil
...
flyBtn.MouseButton1Click:Connect(function()
    ...
    if r and h then
        if flying then
            ...
            -- สร้างระบบฟิสิกส์สมัยใหม่ (LinearVelocity & AlignOrientation)
            att = Instance.new("Attachment", r)
            
            lv = Instance.new("LinearVelocity", r)
            ...
            ao.Attachment0 = att
            ao.Mode = Enum.OrientationMode.OneAttachment
        else
            disableFlying()
...
R.RenderStepped:Connect(function()
    ...
    -- [ฟิกซ์ระบบฟิสิกส์สมัยใหม่]
    if flying and lv and ao then
        -- สั่งให้หน้าตัวละครล็อกตามมุมกล้องราบตลอดเวลา 
        local lookDir = C.CFrame.LookVector
        local flatLookAt = Vector3.new(lookDir.X, 0, lookDir.Z).Unit
        ao.CFrame = CFrame.new(Vector3.new(), flatLookAt)

        -- ตรวจสอบปุ่มบินเสริมคอมค้างไว้
        ...
        -- เช็คการดันจอยสติ๊กจากหน้าจอมือถือ
        if hum.MoveDirection.Magnitude > 0 or verticalMove ~= 0 then
            -- สั่งให้พุ่งตัวตามจอยสติ๊กมือถือ และล็อกทิศความเร็วตามทิศทางกล้องจริง
            local moveDir = hum.MoveDirection * flySpeed
            local vertDir = Vector3.new(0, verticalMove * flySpeed, 0)
            lv.VectorVelocity = moveDir + vertDir
        else
            -- ปล่อยปุ่ม/ปล่อยจอย ล็อกความเร็วเป็น 0 หยุดนิ่งสนิทกลางอากาศทันที
            lv.VectorVelocity = Vector3.new(0, 0, 0)
        end
    end
    ...
    if following and targetName ~= "" then
        ...
        if targetP and targetP.Character and targetP.Character:FindFirstChild("HumanoidRootPart") then
            ...
            if flying and lv then lv.VectorVelocity = Vector3.new(0,0,0) end
...
-- เคลียร์ระบบฟิสิกส์กรณีตัวละครตาย
