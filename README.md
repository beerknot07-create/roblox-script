...
-- ป้องกันหน้าต่างซ้อน (ถ้ามีอันเก่าค้างอยู่ ลบออกทันที)
if L.PlayerGui:FindFirstChild("ZxtataGui") then
    L.PlayerGui.ZxtataGui:Destroy()
end
...
tH.Size, tH.BackgroundColor3, tH.Text, tH.TextColor3, tH.Font, tH.TextSize = UDim2.new(1, -30, 0, 35), Color3.fromRGB(35, 35, 35), "  zxtata | เมนูหลัก (360°)", Color3.fromRGB(255, 255, 255), Enum.Font.SourceSansBold, 14
...
-- ปุ่มหมวดหมู่หลัก
local flyBtn = createBtn(mainContainer, "ระบบ บินอิสระ 360°: ปิดอยู่", 10)
...
flyBtn.MouseButton1Click:Connect(function()
    ...
    if r and h then
        if flying then
            ...
            flyPlatform.CFrame = r.CFrame * CFrame.new(0, -3.5, 0)
...
-- ลูปการทำงานเบื้องหลัง (ระบบบิน 360 องศาเต็มรูปแบบ)
R.RenderStepped:Connect(function(dt)
    ...
    -- [ระบบบิน 360 องศา: ก้มเงยตามทิศทางกล้องอิสระ]
    if flying and flyPlatform then
        hum.PlatformStand = true
        
        -- ดึงค่ามุมมองกล้องแบบ 360 องศาเต็มพิกัด (รวมแกนก้มและเงย Y)
        local camCFrame = C.CFrame
        local moveDir = hum.MoveDirection
        
        -- คำนวณเวกเตอร์พุ่งไปตามทิศทางกล้องและจอยสติ๊กมือถือ
        local flyDirection = Vector3.new(0, 0, 0)
        if moveDir.Magnitude > 0 then
            -- แปลงทิศทางเดินตามมุมกล้อง (รองรับทั้งเดินหน้า ถอยหลัง ซ้าย ขวา และพุ่งเฉียง)
            flyDirection = (camCFrame.RightVector * moveDir.X + camCFrame.LookVector * -moveDir.Z).Unit
        else
            -- ถ้าไม่ดันจอยสติ๊ก แต่ถ้ากดปุ่มกระโดด ค้างไว้จะให้พุ่งขึ้นตามมุมกล้อง หรือหยุดนิ่ง
            if U:IsKeyDown(Enum.KeyCode.Space) then
                flyDirection = camCFrame.LookVector
            end
        end

        -- ขยับแท่นล่องหนไปตามทิศทาง 360 องศา
        flyPlatform.CFrame = flyPlatform.CFrame + (flyDirection * flySpeed * dt)
        
        -- ล็อกตัวละครให้หันหน้าตรงตามกล้องและยืนบนแท่นตลอดเวลา
